...
if [ -f $wb_ap_connection ]; then
    local tmp_connection="/tmp/wb-ap.nmconnection"
    ...
    local MD5=$(md5sum "$tmp_connection" | cut -f 1 -d ' ')
...
if [ ! -f "$wb_ap_connection" ] && [ ! -f "$deleted_wb_ap" ]; then
    local wb_ap_connection_template="/usr/lib/NetworkManager/system-connections/wb-ap.nmconnection"
